  ...
  return (
    <main className="hazard-grid min-h-screen bg-ink px-5 py-10 text-zinc-100">
      <section className="mx-auto max-w-5xl">
        ...
        <CardActions imageUrl={`/report/${encodeURIComponent(report.owner)}/share-image`} score={report.score} />

        <article className="relative mt-6 overflow-hidden rounded-[2rem] border border-hazard/30 bg-[#080905] shadow-[0_0_0_1px_rgba(255,212,0,0.08),0_40px_120px_rgba(0,0,0,0.55)]">
...
async function loadCardReport(owner: string): Promise<CardReport> {
  ...
  return {
    owner: stored.owner,
    score: payload.score,
    status: exposureStatus(payload.score),
    phrase: exposurePhrase(payload.score),
